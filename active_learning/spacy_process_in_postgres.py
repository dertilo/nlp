...
if __name__ == '__main__':
    ...
    with sqlalchemy_engine.connect() as conn:
        for batch in fetchmany_sqlalchemy(sqlalchemy_engine, q, batch_size=10):
            ...
            ids_sentences = [{'id':eid.replace('/','').replace(':',''),'tokenizd_sentences':process_to_str(doc)} for eid, doc in spacy_docs]
            insert_or_update(conn,arxiv_table,['tokenized_sentences'],ids_sentences)
    print('old method took: %0.2f secs'%(time()-start))
    def process_fun(batch):
        spacy_docs = [(d['id'], spacy_nlp(json.loads(d['pdf_full_text']))) for d in batch]
        processed_docs = [{'id': eid.replace('/', '').replace(':', ''), 'tokenized_sentences': process_to_str(doc)} for eid, doc in
                         spacy_docs]
        return processed_docs

    start = time()
    update_batchwise(sqlalchemy_engine, select([arxiv_table]), arxiv_table, process_fun, batch_size=batch_size)
