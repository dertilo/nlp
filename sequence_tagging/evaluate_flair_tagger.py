...
from sequence_tagging.seq_tag_util import spanwise_pr_re_f1, bilou2bio, calc_seqtag_tokenwise_scores


# following method might be deprecated
# def calc_train_test_spanwise_f1(tagger:SequenceTagger, train_sentences, test_sentences):
#     tag_name = tagger.tag_type
...
#
#     pred_data = flair_tagger_predict_bio(tagger, tag_name, train_sentences)
#     pprint('train-f1-macro: %0.2f' % calc_seqtag_tokenwise_scores(gold_targets_train, pred_data)['f1-macro'])
#     pprint('train-f1-micro: %0.2f' % calc_seqtag_tokenwise_scores(gold_targets_train, pred_data)['f1-micro'])
#     _, _, f1 = spanwise_pr_re_f1(pred_data, gold_targets_train)
#     pprint('train-f1-spanwise: %0.2f' % f1)
#
#     pred_data = flair_tagger_predict_bio(tagger, tag_name, test_sentences)
#     pprint('test-f1-macro: %0.2f' % calc_seqtag_tokenwise_scores(gold_targets_test, pred_data)['f1-macro'])
#     pprint('test-f1-micro: %0.2f' % calc_seqtag_tokenwise_scores(gold_targets_test, pred_data)['f1-micro'])
...
def flair_tagger_predict_bio(tagger:SequenceTagger, sentences:List[Sentence]):
...
def evaluate_sequence_tagger(model:SequenceTagger,
    ...
    with torch.no_grad():
        ...
        scores = calc_seqtag_tokenwise_scores(gold_seqs, pred_seqs)
