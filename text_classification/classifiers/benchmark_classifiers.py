...
if __name__ == '__main__':
    ...
    data = [data[i] for i in numpy.random.random_integers(0,len(data)-1,size=(10_000,))]
    label_counter = Counter([d['labels'][0] for d in data])
    pprint(label_counter)

    benchmark(TfIdfSGDSklearnClf, parameters={'text_to_bow_fun':raw_bow, 'alpha':0.00001}, data=data)
    ...
    model_cfg = BertConfig(
        ...
        dim=128,dim_ff=4*128,n_layers=2)
    max_len = 128
    set_seeds(cfg.seed)
